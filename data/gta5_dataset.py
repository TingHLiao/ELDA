...
class GTA5DataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        edge = cv2.imread(datafiles["img"], cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
    dst = GTA5DataSet('./data/GTA5/',list_path = './gta5_list/train.txt')
    trainloader = data.DataLoader(dst, batch_size=1)
    for i, data in enumerate(trainloader):
        imgs, labels, size, edge = data
        labels = labels.cpu().data.numpy()
        labels = labels.transpose(1,2,0).squeeze()
        if i == 0:
            print(labels.shape)
            print(labels[0][0])
        assert False

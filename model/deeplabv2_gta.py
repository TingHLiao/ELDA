...
import torch
import numpy as np
from model.layers import SABlock
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, ema=False):
        # model, block=Bottleneck, layers=[3, 4, 23, 3]
        self.inplanes = 64
        self.ema = ema
...
def Res_Deeplab(num_classes, ema=False):
    model = ResNet(Bottleneck,[3, 4, 23, 3], num_classes, ema=ema)
    return model

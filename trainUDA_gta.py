...
from utils.loss import BerhuLoss, BinaryDiceLoss, WeightedFocalLoss, MultiTaskLoss
...
def create_ema_model(model):
    ...
    ema_model = Res_Deeplab(num_classes=num_classes, ema=True)
...
def main():
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        predD = torch.sigmoid(interp(out["D_src"])).squeeze()
        predD_init = torch.sigmoid(interp(out["initial_D_src"])).squeeze()
        
        L_E_l = loss_edge_calc(predD_init, edge) + loss_edge_calc(predD, edge)
        L_S_l = loss_calc(pred, labels) + loss_calc(pred_init, labels)
        # print("S_init", loss_calc(pred_init, labels), "S", loss_calc(pred, labels))
        # print("E_init", loss_edge_calc(predD_init, edge), "E", loss_edge_calc(predD, edge))
     
        L_l = edge_loss_weight * L_E_l + L_S_l
        print("labeled loss", L_l)
        ...
        if train_unlabeled:
            ...
            if consistency_loss == 'MSE':
                ...
                edge_pred_ui = torch.sigmoid(edge_pred_ui).squeeze()
                ...
                L_u = L_S_u + edge_loss_weight * L_E_u
...
if __name__ == '__main__':
    ...
    edge_loss_weight = config['training']['edge_loss_weight']
